	...
	$.ajax({
		...
		success: function(resultado){
			...
			if(resultado.executora_id>0){
				...
				carregaRegioes(resultado.cidade_id);
...
function cadastraCasaExecutora(){
	
	var id = $("#hidIdExecutora").val();
	
	$.ajax({
	  type: "POST",
	  url: "https://portal.seds.sp.gov.br/coed/public/componentes/cadastro-executora/model/cadastraCasa.php",
	  data: {'id':id},
	  success: function () {
		listaCasas(id);
	  }
...
function carregaRegioes(id){
	$.ajax({
		url:'https://portal.seds.sp.gov.br/coed/public/componentes/cadastro-executora/model/carregaRegioes.php',
		dataType: 'JSON',
		type: 'POST',
		data: {'id':id},
		success: function(resultado){
			$("#boxRegiaoAdministrativa").html(resultado.regiao_administrativa_descricao);
			$("#boxRegiaoMetropolitana").html(resultado.regiao_metropolitana_descricao);
			$("#boxMacroregiao").html(resultado.macroregiao_descricao);
		},
		complete: function(){}
	 });
}
...
function listaCasas(id){
	$.ajax({
		type: "POST",
		url: "https://portal.seds.sp.gov.br/coed/public/componentes/cadastro-executora/model/listaCasas.php",
		data: {"id":id},
		success: function (retorno) {
		  $("#boxListaCasas").html(retorno);
		}
    });
}
