...
$sistema->select("rec_celebrantes","celebrante_valor_previsto_rh , celebrante_valor_previsto_custeio , celebrante_valor_previsto_terceiros","celebrante_id = " . $_POST["id"]);
...
$where = 'a.tipo_repasse_id = 1 AND cabecalho_mes_referencia = "' . $dataRef . '" AND prestacao_mes_referencia = "' . $dataRef . '" AND a.celebrante_id = ' . $_POST["id"] . ' AND b.celebrante_id = ' . $_POST["id"];
...
if(count($resultCabecalho)>0){
    ...
    $valorCusteioPrevistoFixo = str_replace("," , "." , str_replace("." , "" , $repasse[0]["celebrante_valor_previsto_custeio"]));
    $valorRHPrevistoFixo = str_replace("," , "." , str_replace("." , "" , $repasse[0]["celebrante_valor_previsto_rh"]));
    $valorTerceirosPrevistoFixo = str_replace("," , "." , str_replace("." , "" , $repasse[0]["celebrante_valor_previsto_terceiros"]));
    ...
    $dados->{'previstoFixoCusteio'} = $repasse[0]["celebrante_valor_previsto_custeio"];
    $dados->{'previstoFixoRH'} = $repasse[0]["celebrante_valor_previsto_rh"];
    $dados->{'previstoFixoTerceiros'} = $repasse[0]["celebrante_valor_previsto_terceiros"];
    ...
    $dados->{'previstoFixoRH'} = $repasse[0]["celebrante_valor_previsto_rh"];
    $dados->{'previstoFixoTerceiros'} = $repasse[0]["celebrante_valor_previsto_terceiros"];
    $dados->{'totalPrevistoCusteio'} = $repasse[0]["celebrante_valor_previsto_custeio"];
    $dados->{'totalPrevistoRH'} = $repasse[0]["celebrante_valor_previsto_rh"];
    $dados->{'totalPrevistoTerceiros'} = $repasse[0]["celebrante_valor_previsto_terceiros"];
    $result = json_encode($dados);
